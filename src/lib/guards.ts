...
export async function requireUser(locale: Locale = DEFAULT_LOCALE): Promise<User> {
...
export async function requireSiteAccess(
  ...
  if (!allowedRoles.includes(row.role as Role)) redirect(`/${locale}/dashboard/${siteId}`);

  const flags = await db.select().from(featureFlags).where(eq(featureFlags.siteId, siteId));
  const features = {
    blog: false,
    catalog: false,
    quote: false,
  ...
  return { user, site: row.site, role: row.role as Role, features, theme: row.theme };
}
