  ...
  return (
    <div className="max-w-3xl">
      ...
      <div className="space-y-5">
          ...
          <CardBody className="space-y-4">
            ...
            <Field label={t.settings.subdomain} hint={t.settings.subdomainHelp}>
              <div className="flex items-center gap-2 rounded-xl bg-zinc-50 px-3.5 py-2.5 text-sm">
                <span className="font-mono text-zinc-900">{site.subdomain}</span>
            ...
            <div className="grid gap-3 sm:grid-cols-4">
              {DEFAULT_THEMES.map((theme) => (
                ...
                >
                  ...
                  <p className="px-2.5 py-2 text-[12.5px] font-medium">{theme.name}</p>
                </button>
              ))}
            </div>
          </CardBody>
                      ...
                      onClick={() => {
                        ...
                        setForm({
                          ...
                          supportedLanguages: active
                            ? form.supportedLanguages.filter((value) => value !== code)
                            : [...form.supportedLanguages, code],
                        });
                      }}
                      className={cn(
                        "rounded-xl border-2 px-4 py-2 text-[13px] font-medium transition",
                        active ? "border-zinc-900 bg-zinc-50" : "border-zinc-200 text-zinc-500",
                        isDefault && "cursor-default opacity-70"
                      )}
                    >
                      {LOCALE_LABELS[code]}
                      {isDefault && <span className="ml-1.5 text-[11px]">★</span>}
                    </button>
                  );
                })}
              </div>
        ...
        <Card>
          <CardHeader title={t.settings.modules} description={t.settings.modulesHelp} />
          <CardBody className="divide-y divide-zinc-100">
            {FEATURES.map((feature) => (
              <div key={feature} className="flex items-center justify-between gap-4 py-3 first:pt-0 last:pb-0">
                <div>
                  <p className="text-[13.5px] font-medium text-zinc-900">
                    {featureLabels[feature].name}
                  </p>
                  <p className="text-[12.5px] text-zinc-500">{featureLabels[feature].desc}</p>
                </div>
                <Switch
        ...
        <Card>
          <CardHeader title={t.settings.visibility} description={t.settings.visibilityHelp} />
          <CardBody>
            <div className="flex gap-2">
        ...
        {canDelete && (
          <Card className="border-red-200">
            <CardHeader title={t.settings.dangerZone} description={t.settings.deleteSiteHelp} />
            <CardBody>
