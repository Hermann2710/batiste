        ...
        <section className="relative overflow-hidden" style={{ background: "var(--c-surface)" }}>
          ...
          >
            ...
            {str(content.subtitle) && (
              <p
                data-anim="up" data-delay="0.1"
                className={`mt-5 max-w-2xl text-[16px] leading-relaxed opacity-80 ${
                  align === "left" ? "" : "mx-auto"
                }`}
              >
                {str(content.subtitle)}
        ...
        <Section surface>
          ...
          <div className={`grid gap-5 ${gridClass}`}>
            {list(content.cards).map((card, index) => (
              <div key={index} data-anim="up" data-delay={String(index * 0.08)} className="site-card overflow-hidden transition-transform hover:-translate-y-1 hover:shadow-lg">
                {str(card.imageUrl) && (
        ...
        >
          <div className="mx-auto max-w-2xl">
            ...
            {str(content.description) && (
              <p data-anim="fade" data-delay="0.1" className="mt-3 text-[15px] opacity-85">{str(content.description)}</p>
            )}
            {str(content.buttonText) && (
              <a
        ...
        <Section>
          ...
          <div className="grid gap-5 sm:grid-cols-2 lg:grid-cols-3">
            {list(content.items).map((item, index) => (
              <figure key={index} data-anim="up" data-delay={String(index * 0.1)} className="site-card p-6 transition-transform hover:-translate-y-1 hover:shadow-md">
          ...
          {items.length === 0 ? (
            ...
            <div className={`grid gap-5 ${gridClass}`}>
              {items.map((product, idx) => {
                ...
                return (
                  <article key={product.id} data-anim="up" data-delay={String(idx * 0.07)} className="site-card overflow-hidden transition-transform hover:-translate-y-1 hover:shadow-lg">
                    {image ? (
                      // eslint-disable-next-line @next/next/no-img-element
